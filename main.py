    ...
    async with db_pool.acquire() as conn:
        messages = await conn.fetch(
            ...
            translation_data AS (
                SELECT 
                    t.message_id,
                    json_object_agg(t.language_code, t.translated_content) as translations
                FROM translations t
            ...
            SELECT 
                ...
                COALESCE(td.translations, '{}'::json) as translations
            FROM messages m
            JOIN users u1 ON m.sender_id = u1.id
            JOIN users u2 ON m.recipient_id = u2.id
            LEFT JOIN pinned_messages pm ON m.id = pm.message_id AND pm.user_id = $1
            LEFT JOIN message_reactions mr ON m.id = mr.message_id
            LEFT JOIN translation_data td ON m.id = td.message_id
        ...
        for msg in messages:
            ...
            translations = msg["translations"] if msg["translations"] else {}
...
# Translate message
@app.post("/messages/translate/{message_id}")
async def translate_message(message_id: int, request: TranslationRequest, current_user: dict = Depends(get_current_user)):
    async with db_pool.acquire() as conn:
        msg = await conn.fetchrow(
            """
            SELECT m.id, m.content, m.sender_id, m.recipient_id,
                   u1.username AS sender_username,
                   u2.username AS recipient_username
            FROM messages m
            JOIN users u1 ON m.sender_id = u1.id
        ...
        existing_translation = await conn.fetchrow(
            """
            SELECT translated_content
            FROM translations
            WHERE message_id = $1 AND language_code = $2
            """,
            message_id, request.language_code
        )
        
        if existing_translation:
            return {"translated_content": existing_translation["translated_content"]}
        ...
        target_language = language_map.get(request.language_code, request.language_code)
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    ...
                    json={
                        ...
                        "messages": [{
                            "role": "user",
                            "content": f"Translate \"{msg['content']}\" from English to {target_language}."
                        }]
                    }
                )
                ...
                if response.status_code != 200:
                    raise HTTPException(
                        status_code=500,
                        detail=f"Translation service unavailable (Status: {response.status_code})"
                    )
                ...
                if not translated_text:
                    raise HTTPException(status_code=500, detail="No translated text received from service")
                
                await conn.execute(
                    """
                    INSERT INTO translations (message_id, language_code, translated_content)
                    VALUES ($1, $2, $3)
                    """,
                    message_id, request.language_code, translated_text
                )
                ...
                if msg["sender_username"] in connections:
                    await connections[msg["sender_username"]].send_json({
                        "type": "translation",
                        "data": translation_data
                    })
                if msg["recipient_username"] in connections:
                    await connections[msg["recipient_username"]].send_json({
                        "type": "translation",
                        "data": translation_data
                    })
                
                logger.info(f"Message {message_id} translated to {request.language_code} by {current_user['username']}")
        ...
        try:
            ...
            async with conn.transaction():
                ...
                reactions = await conn.fetch(
                    """
                    SELECT r.emoji, COUNT(*) as count, json_agg(u.username) as users
                    FROM reactions r
                    JOIN users u ON r.user_id = u.id
...
async def get_api_key(current_user: dict = Depends(get_current_user)):
    return {"api_key": "4e8SqXXNZmFjG4AoKFGmuYLbPpwenRAH"}
